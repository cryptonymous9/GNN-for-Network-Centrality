...
def margin_ranking(y_out,true_val,node_num):
    y_out = y_out.reshape((model_size))
    true_val = true_val.reshape((model_size))
    predict_arr = y_out.cpu().detach().numpy()
    true_arr = true_val.cpu().detach().numpy()
    
    predict_arr_tensor = torch.tensor(predict_arr)
    true_arr_tensor = torch.tensor(true_arr)
    MR_loss = MarginRankingLoss(predict_arr_tensor, true_arr_tensor)
    return MR_loss
